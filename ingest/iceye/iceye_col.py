...
def filter_latest_revisions(event_paths):
    ...
    for path in event_paths:
        ...
        if fsd_id:
            events_by_fsd[fsd_id].append({"path": path, "revision": revision, "event_dir": event_dir})
        else:
            # If we can't parse FSD ID, keep the event anyway
            logging.warning(f"Could not parse FSD ID from {event_dir}, including anyway")
            events_by_fsd[event_dir].append({"path": path, "revision": 0, "event_dir": event_dir})
...
def process_event(event_path, s3_utils, bucket_name, link_type, collection, reprocess_assets, asset_handler):
...
def create_item(event_path, event_id, asset_results, s3_utils, bucket_name, link_type, collection):
    ...
    if depth_unit_info:
        ...
        if depth_unit_info.get("conversion_factor", 1.0) != 1.0:
            ...
            properties["iceye:depth_conversion_note"] = (
                f"Original depth values in {depth_unit_info['original_unit']} multiplied by {depth_unit_info['conversion_factor']} to standardize to inches"
            )
    ...
    # Add flowfile properties if available
    if asset_results.get("flowfile_object"):
        properties["flowfiles"] = asset_results["flowfile_object"]
...
def create_assets(item, event_id, asset_results, s3_utils, bucket_name, link_type):
    ...
    for idx, thumbnail_path in enumerate(thumbnail_paths):
        ...
        if is_valid:
            ...
            item.add_asset(
                asset_id, pystac.Asset(href=thumbnail_href, media_type="image/png", roles=["thumbnail"], title=title)
            )
        else:
        ...
        if release_path.endswith(".pdf"):
            ...
            if is_valid:
                item.add_asset(
                    asset_id,
                    pystac.Asset(
                        href=asset_href, media_type="application/pdf", roles=["metadata"], title="Release Notes"
                    ),
                )
            else:
    ...
    for metadata_path in asset_paths.get("flood_metadata", []):
        ...
        if is_valid:
            item.add_asset(
                asset_id,
                pystac.Asset(
                    href=asset_href, media_type="application/json", roles=["metadata"], title="Flood Metadata"
                ),
            )
        else:
    ...
    # Add flowfile CSV
    flowfile_key = asset_results.get("flowfile_key")
    if flowfile_key:
        asset_id = "NWM_ANA_flowfile"
        flowfile_href, is_valid = s3_utils.generate_href(bucket_name, flowfile_key, link_type)
        if is_valid:
            item.add_asset(
                asset_id,
                pystac.Asset(
            ...
            logging.warning(f"Skipping flowfile asset {flowfile_key} - invalid or inaccessible")
...
def main():
    ...
    # Load NWM hydrofabric for flowfile creation
    nwm_streams_s3_path = "hand_fim/inputs/nwm_hydrofabric/nwm_flows.gpkg"
    script_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(script_dir)
    nwm_streams_local_path = os.path.join(parent_dir, "nwm_flows.gpkg")

    # Download if not exists
    if not os.path.exists(nwm_streams_local_path):
        logging.info(f"Downloading NWM hydrofabric from s3://{args.bucket_name}/{nwm_streams_s3_path}")
        s3_utils.s3_client.download_file(args.bucket_name, nwm_streams_s3_path, nwm_streams_local_path)
        logging.info("NWM hydrofabric downloaded")
    else:
        logging.info("Using existing NWM hydrofabric")

    # Load hydrofabric into GeoDataFrame
    logging.info("Loading NWM hydrofabric into memory...")
    nwm_flows_gdf = gpd.read_file(nwm_streams_local_path)
    logging.info(f"Loaded {len(nwm_flows_gdf)} NWM features")
    ...
    asset_handler = ICEYEAssetHandler(s3_utils, args.bucket_name, args.derived_metadata_path, nwm_flows_gdf)
