...
class BLEAssetHandler:
    ...
    def create_flowfile_object(self, huc8_path, magnitudes):
        ...
        flowfile_dfs = FlowfileUtils.download_flowfiles(
            self.bucket_name, list(flowfile_keys.values()), self.s3_utils.s3_client
        )
        ...
        flowfile_object = FlowfileUtils.create_flowfile_object(
            list(flowfile_ids.values()), flowstats_list, columns_list
        )
    ...
    def write_data_parquet(self, results):
        ...
        new_df = (
            pd.DataFrame.from_dict(results_copy, orient="index").reset_index().rename(columns={"index": "huc8_path"})
        )
