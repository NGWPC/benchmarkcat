...
from ingest.gauge.gauge_handle_assets import GaugeFIMAssetHandler
from ingest.gauge.gauge_stac import GaugeFIMInfo, GeoJSONHandler, AssetUtils
...
agency = extract_agency(asset_object_key)
agency
...
def create_gauge_fim_collection(agency):
    ...
    item_assets_ext.item_assets = GaugeFIMInfo.assets
...
def create_assets(item, gauge_path, gauge, asset_results, s3_utils, bucket_name, link_type):
    ...
    for magnitude in asset_results["magnitudes"]:
        ...
        for flowfile_key in asset_results["flowfile"]["flowfile_keys"]:
            item.add_asset(
...
def main():
    ...
    collection = create_gauge_fim_collection(asset_agency)
    huc8_paths = get_huc8_paths(s3_utils, args.bucket_name, args.asset_object_key)
    asset_handler = GaugeFIMAssetHandler(s3_utils, args.bucket_name, args.derived_metadata_path,asset_agency)
