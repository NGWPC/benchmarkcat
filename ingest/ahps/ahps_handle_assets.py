...
class AHPSFIMAssetHandler:
    ...
    def handle_assets(self, gauge_path) -> Dict[str, Any]:
        ...
        magnitude_paths = self.s3_utils.list_resources_with_string(
            self.bucket_name, gauge_path, ["minor", "major", "moderate", "action"], delimiter="/"
        )
    ...
    def create_flowfile_object(self, magnitude_paths, magnitudes):
        ...
        # Use columns_list from AHPSFIMInfo
        columns_list = AHPSFIMInfo.columns_list
    ...
    def write_data_parquet(self, results):
        ...
        new_df = (
            pd.DataFrame.from_dict(results_copy, orient="index").reset_index().rename(columns={"index": "gauge_path"})
        )
