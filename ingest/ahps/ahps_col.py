...
from ingest.ahps.ahps_stac import AHPSFIMInfo, AssetUtils, GeoJSONHandler
...
def parse_arguments():
    ...
    parser.add_argument(
        "--asset_object_key",
        type=str,
        default="hand_fim/test_cases/usgs_test_cases/validation_data_usgs/",
        help="Key for the asset object in the S3 bucket",
    )
    parser.add_argument(
        "--reprocess_assets", action="store_true", help="Set to true to reprocess assets using USGSFIMAssetHandler"
    )
    # derived_metadata_path agency needs to match the agency in "asset_object_key"
    parser.add_argument(
        "--derived_metadata_path",
        type=str,
        default="benchmark/stac-bench-cat/assets/derived-asset-data/usgs_fim_collection.parquet",
...
def validate_agencies(asset_agency, metadata_agency):
    if asset_agency != metadata_agency:
        raise ValueError(
            f"Agency mismatch: '{asset_agency}' from asset_object_key does not match '{metadata_agency}' from derived_metadata_path."
        )
...
def process_huc8(huc8_path, s3_utils, bucket_name, link_type, collection, agency, reprocess_assets, asset_handler):
    ...
    for gauge_path in s3_utils.list_subdirectories(bucket_name, huc8_path):
        process_gauge(
            gauge_path, agency, huc8, s3_utils, bucket_name, link_type, collection, reprocess_assets, asset_handler
        )
...
def create_assets(item, gauge_path, gauge, asset_results, s3_utils, bucket_name, link_type):
    ...
    for magnitude in asset_results["magnitudes"]:
        ...
        flowfile_key = next(key for key in asset_results["flowfile"]["flowfile_keys"] if magnitude in key)
