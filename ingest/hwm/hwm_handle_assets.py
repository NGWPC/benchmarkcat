...
class HWMAssetHandler:
    ...
    def get_flowfile_object(self, event_id, points, flowfile_dir):
        ...
        if event_month and datetime(1979, 2, 1) <= event_month <= datetime(2023, 1, 31):
            ...
            if not flowfile_df.empty and "discharge" in flowfile_df.columns:
                ...
                return (
                    event_month,
                    FlowfileUtils.create_flowfile_object(flowfile_ids, flowstats, hwm_stac.columns_list),
                    flowfile_key,
                )
            else:
    ...
    def write_data_parquet(self, results):
        ...
        new_df = (
            pd.DataFrame.from_dict(results_copy, orient="index").reset_index().rename(columns={"index": "event_id"})
        )
    ...
    def open_ds(self, url):
        return xr.open_zarr(fsspec.get_mapper(url, anon=True), consolidated=True, mask_and_scale=True)[
            "streamflow"
        ].drop_vars(["latitude", "elevation", "gage_id", "longitude", "order"])
    ...
    def extract_date_from_event_id(self, event_id):
        ...
        if match:
            ...
            month_dict = {
                **{m.lower(): i for i, m in enumerate(month_abbr) if m},
