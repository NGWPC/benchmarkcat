...
Migrates STAC catalog and assets from NGWPC S3 (`fimc-data`) to OWP S3 (`owp-benchmark`) with reorganized structure.
...
Run from EC2 in OWP account with an IAM role that has write access to `owp-benchmark` and read access to `fimc-data`.
...
```bash
aws s3 ls s3://your-test-bucket/data/iceye-collection/ --recursive
```
...
- → `s3://owp-benchmark/data/gfm-collection/dfo-4336/flood.tif`
...
| Permission errors | Verify access: `aws s3 ls s3://fimc-data/benchmark/ --profile your-profile` and `aws s3 ls s3://owp-benchmark/ --profile your-profile`. Check identity with `aws sts get-caller-identity`. |
